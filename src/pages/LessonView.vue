<template>
  <section :style="{ display: 'flex', flexDirection: 'column', gap: 'var(--md-sys-spacing-6)' }">
    ...
    >
      ...
      <component :is="markdownComponent" class="lesson-content prose max-w-none dark:prose-invert" />
    </article>

    <article v-else class="card max-w-none p-8 text-center text-body-medium text-[var(--md-sys-color-on-surface-variant)]">
...
const lessonModules = import.meta.glob('../content/courses/*/lessons/*.md');
...
async function loadLesson() {
  ...
  try {
    ...
    const loader = async () => {
      ...
      lessonTitle.value = mod.frontmatter?.title ?? entry.title;
      lessonObjective.value = mod.frontmatter?.objective ?? entry.description ?? '';
      await nextTick();
      highlight();
      return mod.default;
    };
