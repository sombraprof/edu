<template>
  <div class="card p-8 my-10">
    ...
    <p
      ...
      class="text-body-large mb-8 text-[var(--md-sys-color-on-surface-variant)]"
    >
      {{ data.description }}
    </p>
    ...
    <div class="card surface-tonal p-8">
      <div class="relative border-l-2 border-[var(--md-sys-color-outline)] ml-5">
        <ol
          class="pl-10"
          :style="{ display: 'flex', flexDirection: 'column', gap: 'var(--md-sys-spacing-10)' }"
        >
          <li v-for="(step, index) in data.steps" :key="index" class="relative">
            ...
            <h4
              class="font-bold md-sys-typescale-headline-small text-[var(--md-sys-color-primary)]"
            >
              {{ step.title }}
            </h4>
            <p
              class="mt-1 text-body-medium text-[var(--md-sys-color-on-surface-variant)]"
              v-html="step.content"
            ></p>
          </li>
        </ol>
