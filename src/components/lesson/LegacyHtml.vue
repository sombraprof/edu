<template>
  <section class="legacy-html lesson-content prose max-w-none dark:prose-invert" v-html="sanitizedHtml"></section>
</template>
...
function stripRawHtml(html: string): string {
  ...
  try {
    if (typeof window !== 'undefined' && typeof window.DOMParser !== 'undefined') {
      ...
      doc.querySelectorAll('script, style, link[rel="stylesheet"], noscript, iframe, meta').forEach((node) => node.remove());
...
.legacy-html {
  ...
  font-family: var(--md-sys-typescale-body-large-font, 'Inter', system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif);
  line-height: 1.6;
}
...
.legacy-html :deep(h1) { font-size: var(--md-sys-typescale-headline-medium-size, 1.75rem); }
.legacy-html :deep(h2) { font-size: var(--md-sys-typescale-title-large-size, 1.375rem); }
.legacy-html :deep(h3) { font-size: var(--md-sys-typescale-title-medium-size, 1.125rem); }
.legacy-html :deep(h4),
.legacy-html :deep(h5),
.legacy-html :deep(h6) { font-size: var(--md-sys-typescale-title-small-size, 1rem); }
