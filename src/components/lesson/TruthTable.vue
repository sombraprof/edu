    ...
    <div class="overflow-x-auto -mx-2 sm:mx-0">
      <table class="w-full truth-table min-w-[480px]">
        <thead class="bg-surface-variant text-on-surface-variant">
          <tr>
            <th v-for="(header, index) in headers" :key="index" class="p-3 text-left font-semibold">
              {{ header }}
            </th>
          </tr>
        </thead>
        <tbody>
          ...
          >
            <td
              ...
              :key="cellIndex"
              class="p-3 text-on-surface"
              :class="{ 'result true': cell.isTrue, 'result false': cell.isFalse }"
            >
