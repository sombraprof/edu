/**
 * Sample usage for the didactic components.
 *
 * Provides realistic data sets that demonstrate how each component renders
 * inside lessons and exercises.
 */

import { createTruthTable, createFlowchart, createBlockDiagram } from './index';

// Example 1: Truth table for the AND operator
export const truthTableExample = createTruthTable({
  title: 'Tabela Verdade - Operador E (AND)',
  description:
    'O operador <strong>E</strong> resulta em verdadeiro apenas quando ambas as condições são verdadeiras.',
...
// Example 2: Flowchart for a simple grading program
export const flowchartExample = createFlowchart({
  title: 'Fluxograma: Verificação de Aprovação',
  description:
    'Fluxograma que representa a lógica de aprovação de um aluno baseada na média e frequência.',
  nodes: [
    {
      id: 'start',
      type: 'start',
...
// Example 3: Block diagram of a platform
export const blockDiagramExample = createBlockDiagram({
  title: 'Diagrama de Blocos: Sistema de Gerenciamento Escolar',
  description:
    'Representação visual dos componentes principais de um sistema de gerenciamento escolar.',
  blocks: [
    {
      id: 'user',
      type: 'input',
...
// Example 4: Truth table with logical operators
export const logicOperatorsExample = createTruthTable({
  title: 'Tabela Verdade - Operadores Lógicos Básicos',
  description:
    'Comparação entre os operadores <strong>E (∧)</strong>, <strong>OU (∨)</strong> e <strong>NÃO (¬)</strong>.',
...
// Example 5: Flowchart for a sorting algorithm
