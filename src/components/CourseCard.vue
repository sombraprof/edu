<template>
  <router-link
    class="card card--interactive group flex h-full flex-col gap-6 p-6 focus-visible:outline-none"
    :to="{ name: 'course-home', params: { courseId: meta.id } }"
    :aria-label="`Acessar disciplina ${meta.title}`"
  >
    <div class="flex items-start justify-between gap-4">
      <div class="space-y-2">
        <h3 class="text-headline-small font-semibold text-[var(--md-sys-color-on-surface)]">{{ meta.title }}</h3>
    ...
    <div :class="['mt-auto flex w-full items-end gap-4', $slots.meta ? 'justify-between' : 'justify-end']">
      <div v-if="$slots.meta" class="flex flex-wrap gap-2 text-sm text-[var(--md-sys-color-on-surface-variant)]">
        <slot name="meta" />
      </div>
