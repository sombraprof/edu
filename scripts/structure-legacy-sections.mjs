...
function sanitizeSection(section) {
  section.querySelectorAll('script, style, link, iframe, noscript, meta').forEach((node) => node.remove());
...
function convertHtmlToSections(html) {
  ...
  return sections
    .map((section) => extractSectionBlock(section))
...
async function loadLegacyHtml(courseId, lessonId) {
  ...
  if (await exists(legacyJsonPath)) {
    ...
    if (Array.isArray(data.content)) {
      const htmlBlock = data.content.find((block) => block.type === 'html' && typeof block.html === 'string');
      if (htmlBlock?.html) {
